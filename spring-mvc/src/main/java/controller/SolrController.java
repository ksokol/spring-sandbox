...
public class SolrController {
    ...
    private String pathPrefix = "/solr"; // strip this from the beginning of a path
    ...
    public void select(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
        ...
        try {
            ...
            if( path.equals( cores.getAdminPath() ) ) {
                ...
                solrReq =  adminRequestParser.parse(null,path, request);
                handleAdminRequest(request, response, handler, solrReq);
                return;
            }
                ...
                if( handler == null && path.length() > 1 ) { // don't match "" or "/" as valid path
                    ...
                    if( handler == null && parser.isHandleSelect() ) {
                        if( "/select".equals( path ) || "/select/".equals( path ) ) {
                            solrReq = parser.parse( core, path, request );
    ...
    private void handleAdminRequest(HttpServletRequest req, HttpServletResponse response, SolrRequestHandler handler,
    ...
    private void writeResponse(SolrQueryResponse solrRsp, HttpServletResponse response,
    ...
    private int getErrorInfo(Throwable ex, NamedList info) {
        int code=500;
    ...
    private void sendError(SolrCore core,
